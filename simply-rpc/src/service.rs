...
pub trait RestService: Send + Sync {
    ...
    /// Dispatch a request and return typed content parts.
    ///
    /// Unlike `rest_dispatch_by_name` (which erases everything to `Value`),
    /// this preserves type information: `BinaryResponse` → `ContentPart::Binary`,
    /// everything else → `ContentPart::Json`. Generated by the `#[rpc_service]` macro.
    async fn rest_dispatch_as_content(
        &self,
        method_name: &str,
        params: Value,
    ) -> Option<anyhow::Result<Vec<crate::ContentPart>>>;
