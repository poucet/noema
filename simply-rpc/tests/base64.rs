...
use simply_rpc::{
    BinaryResponse, BinaryUpload, HttpMethod, RequestContext, RpcConnection, ServiceRouter,
};
use tokio::sync::mpsc;
...
impl BlobApi for InMemoryBlobs {
    async fn store_blob(&self, upload: BinaryUpload) -> anyhow::Result<BlobId> {
        ...
        self.blobs
            .lock()
            .await
            .push((id.clone(), upload.mime_type, upload.data));
        Ok(id)
    }
...
fn make_remote() -> RemoteBlobApi {
    RemoteBlobApi::new(Arc::new(MockBlobClient::new()))
}

#[tokio::test]
async fn client_store_and_get_blob_round_trip() {
    let client = make_remote();
...
async fn client_store_blob_empty_data() {
    ...
    let id = client
        .store_blob(BinaryUpload::new(vec![], "text/plain"))
        .await
        .unwrap();
    let br = client.get_blob(&id.0).await.unwrap();
...
fn route_meta_for_blob_methods() {
    ...
    let store = meta
        .routes
        .iter()
        .find(|m| m.method_name == "blob.store_blob")
        .unwrap();
    ...
    let get = meta
        .routes
        .iter()
        .find(|m| m.method_name == "blob.get_blob")
        .unwrap();
    ...
    assert!(
        get.immutable_cache,
        "get_blob has immutable_cache annotation"
    );
}
...
async fn start_blob_server() -> (String, Arc<InMemoryBlobs>) {
    ...
    async fn handler(
        ...
        let raw_bytes = match http_method {
            HttpMethod::Post => axum::body::to_bytes(req.into_body(), 10 * 1024 * 1024)
                .await
                .unwrap_or_default(),
            _ => bytes::Bytes::new(),
        };
