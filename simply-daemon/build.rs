...
//!
//! Pipeline:
//! 1. Parse src/api/*.rs for #[rpc_service] traits and #[derive(TS)] structs
//! 2. Generate TypeScript types + API client → admin/src/lib/generated/
...
fn main() {
    ...
    if api_dir.is_dir() && admin_dir.join("package.json").exists() {
        ...
        generate_typescript(&api_dir, &generated_dir);
    }
...
fn generate_typescript(api_dir: &PathBuf, out_dir: &PathBuf) {
    ...
    types.push_str("// Auto-generated TypeScript types from Rust API structs.\n");
    types.push_str("// Do not edit — regenerated by simply-daemon build.rs\n\n");
    ...
    client.push_str("import { json, type Fetch } from '../runtime';\n");
        ...
        for item in &syntax.items {
            if let syn::Item::Struct(s) = item {
                if has_derive(&s.attrs, "Serialize") {
                    if let Some(ts) = struct_to_typescript(s) {
                        type_names.push(s.ident.to_string());
                        types.push_str(&ts);
                        types.push('\n');
                    }
                }
    ...
    fs::write(out_dir.join("types.ts"), &types).expect("failed to write types.ts");
    ...
    println!(
        "cargo:warning=Generated {} types + API clients → {}",
        type_names.len(),
        out_dir.display()
    );
}
...
fn struct_to_typescript(s: &syn::ItemStruct) -> Option<String> {
    let name = &s.ident;
    let syn::Fields::Named(fields) = &s.fields else { return None };

    let mut out = String::new();

    // Doc comment
    if let Some(doc) = extract_doc_comment(&s.attrs) {
    ...
    for field in &fields.named {
        ...
        let ts_name = to_camel_case(&field_name.to_string());
...
fn rust_type_to_ts_from_str(s: &str) -> String {
    ...
    // Complex type — reference from generated types
