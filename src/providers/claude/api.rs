...
pub (crate) struct ListModelsResponse {
    ...
    pub (crate) first_id: Option<String>,
...
impl From<&crate::ChatMessage> for InputMessage {
    fn from(msg: &crate::ChatMessage) -> InputMessage {
        InputMessage {
            ...
            content: vec![Content::Text { citations: None, text: msg.content.clone() }]
        }
    }
}
...
impl SystemPrompt {
    fn new(text: &str) -> Self {
        SystemPrompt::Text { text: text.to_string() }
    }
}
...
impl MessagesRequest {
    pub (crate) fn from_chat_request(model_name: &str, request: &crate::ChatRequest, stream: bool) -> Self {
        ...
        let system_instruction = request.messages.iter()
            ...
            .map(|m| m.content.clone()).collect::<Vec<String>>().join("\n");

        let messages = request.messages.iter().filter(|m| m.role != crate::Role::System)
        ...
        MessagesRequest {
            ...
            max_tokens: 32000,
            stream: Some(stream),
            system: if system_instruction.len() == 0 { None} else {  Some(SystemPrompt::new(&system_instruction)) },
        }
    }
}
...
impl From<MessagesResponse> for crate::ChatMessage {
    fn from(response: MessagesResponse) -> Self {
        response.content.first().expect("No content").try_into().expect("Failed to parse Claude response")
    }
}
