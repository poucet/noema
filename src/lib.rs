...
use llm::api::{ChatChunk, ChatMessage, ChatRequest, Role};
...
impl<'a> Drop for ConversationStream<'a> {
    fn drop(&mut self) {
        if !self.finalized {
            ...
            if !accumulated.content.is_empty() {
...
impl Conversation {
    ...
    pub fn with_system_message(model: impl ChatModel + Send + 'static, system_message: impl Into<String>) -> Self {
    ...
    /// # }
    /// ```
    pub async fn send_stream(&mut self, message: impl Into<String>) -> Result<ConversationStream<'_>> {
        let user_message = message.into();
        self.add_user_message(user_message);
...
mod tests {
    ...
    async fn test_send_stream_accumulates() {
...

        {
            ...
            assert_eq!(stream.accumulated().content, "Hello!");
