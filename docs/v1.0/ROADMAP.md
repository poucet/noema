...
```
Foundation (sequential)               Lumina (12 stages)
  Crate rename → daemon        →      Crate → Chat → Admin → Todo/Notes → Schedule
                                                │       → Voice → RAG → Brain → MCP
...
**Maximum parallelism:** After Lumina Stage 2, four workstreams can advance independently. Lumina stages 3–12 can run in parallel with Content/Events/Voice. Voice library can optionally start during Lumina since it's a standalone crate. RTC starts after Voice Stage 2 (daemon voice API).
