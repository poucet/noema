...
- **URGENT:** Google Docs MCP server moved to ephemeral registration in Lumina, but the OAuth config (client_id, client_secret, oauth_provider) lives in mcp.toml which is for daemon-managed servers. Ephemeral servers bypass mcp.toml so the OAuth flow can't find the credentials. Fix: either (a) Lumina passes OAuth config when registering ephemeral, (b) the daemon reads OAuth config from a separate source for ephemeral servers, or (c) Google Docs stays in mcp.toml as a daemon-managed server (not ephemeral) and Lumina just relies on the daemon to auto-connect it
