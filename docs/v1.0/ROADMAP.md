...
- **URGENT:** Lumina crashes on startup if an MCP server configured in mcp.toml is not running — should gracefully skip unreachable servers and retry later (auto_retry already exists but the initial connect failure is fatal)
...
- Persist OAuth tokens across daemon/Lumina restarts. Currently `TransientTokenStore` is in-memory only — every restart requires re-running `/google auth`. Options: encrypt-at-rest in SQLite (per-user, per-server), or use refresh tokens to silently re-acquire access tokens on startup. Must remain GDPR-safe (easy to purge on user deletion).
- Remove CASCADE reliance from SQLite schema. Currently `DELETE FROM documents` cascades to tabs/revisions via SQL foreign keys. The service layer should handle child deletion explicitly (and already does for embeddings). CASCADE becomes a problem if document storage and tab/chunk storage are backed by different stores (e.g. SQLite for documents, ChromaDB for embeddings/chunks).
- Google Docs import creates one flat document with all tabs concatenated into a single tab. Should create separate document tabs matching the Google Doc's tab structure, using `CreateTabRequest` for each tab after the first.
- Admin UI document page: clicking a document owned by a different user fails because the request context doesn't match. Should auto-switch the user picker to the document's `user_id` when selecting it, so the context is correct without manual switching.
- OAuth identity linking not working end-to-end. The callback fetches the email from the provider's userinfo endpoint and looks up existing users, but the Discord external identity (`discord:{id}`) is not actually remapped to the existing user. The token gets stored under the right user_id but subsequent Discord commands still resolve to the original discord-created user. Need to either (a) create an external identity mapping for the Discord user pointing to the email-matched user, or (b) merge the two user records.
- Document visibility: documents should be either user-owned (private) or public. Public documents (e.g. system prompts, agentic workflow definitions) are accessible to all users and included in everyone's RAG results. The `is_public` field already exists on `Document` but nothing sets or respects it yet. Additionally, lightweight single-content documents (system prompts, markdown configs) shouldn't need the full multi-tab structure — consider a simpler storage path for single-markdown documents that skips the tab indirection.
